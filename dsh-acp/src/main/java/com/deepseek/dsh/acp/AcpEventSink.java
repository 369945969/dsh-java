...
/**
 * ACP session-event sink: appends to SessionLog and logs injection/turn events
 * to stderr via SLF4J (stdout reserved for JSON-RPC frames).
 */
public final class AcpEventSink implements TurnOrchestrator.SessionEventSink {

    private static final Logger log = LoggerFactory.getLogger("dsh.acp.events");

    private final Sessions sessions;

    public AcpEventSink(Sessions sessions) {
    ...
    private void appendToLog(String sessionId, String eventType, Map<String, Object> data) {
        try {
            ...
            log.debug("acp sink append ({}): {}", eventType, e.toString());
        }
    }
